    ...
    async def _verify_all_async(self, grouped_keys: dict) -> list[dict]:
        ...
        async with aiohttp.ClientSession() as session:
            for start in range(0, len(items), self.concurrency):
                ...
                verified = await asyncio.gather(*(self._verify_one(session, key, semaphore) for key, _info in batch))
                ...
                for (key, info), result in zip(batch, verified):
                    ...
                    balance_kind = result.get(
                        "balance_kind", "cash" if not balance_unavailable else ""
                    )
